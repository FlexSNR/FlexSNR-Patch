...
func (tMgr *ThermalManager) Init(logger logging.LoggerIntf, plugin PluginIntf) {
	tMgr.logger = logger
	tMgr.plugin = plugin
	numOfThermals := tMgr.plugin.GetMaxNumOfThermal()
	thermalState := make([]pluginCommon.ThermalState, numOfThermals)
	tMgr.plugin.GetAllThermalState(thermalState, numOfThermals)
	for _, thermal := range thermalState {
		if thermal.ThermalState == "THERMAL NOT PRESENT" {
			continue
		}
		tMgr.thermalIdList = append(tMgr.thermalIdList, ThermalId(thermal.ThermalId))
	}
...
func (tMgr *ThermalManager) GetBulkThermalState(fromIdx int, cnt int) (*objects.ThermalStateGetInfo, error) {
	...
	if fromIdx >= len(tMgr.thermalIdList) {
		return nil, errors.New("Invalid range")
	}
	if fromIdx+cnt > len(tMgr.thermalIdList) {
		retObj.EndIdx = len(tMgr.thermalIdList)
		retObj.More = false
		retObj.Count = 0
	} else {
		retObj.EndIdx = fromIdx + cnt
		retObj.More = true
		retObj.Count = len(tMgr.thermalIdList) - retObj.EndIdx + 1
	}
