...
func (server *ARPServer) updateLagInfra(msg commonDefs.LagNotifyMsg) {
	...
	if msg.MsgType == commonDefs.NOTIFY_LAG_CREATE {
		...
		server.logger.Debug("Received Lag Update Notification IfIdx:", ifIdx, "PortList:", portList)
		newPortMap := make(map[int]bool)
		for i := 0; i < len(portList); i++ {
			port := int(portList[i])
			newPortMap[port] = true
...
func (server *ARPServer) constructLagInfra() {
	...
	count := 100
	for {
		bulkInfo, _ := server.AsicdPlugin.GetBulkLag(curMark, count)
		if bulkInfo == nil {
			break
		}
		objCnt := int(bulkInfo.Count)
		more := bool(bulkInfo.More)
		curMark = int(bulkInfo.EndIdx)
		server.logger.Info("Lag Infra Start! Lag count:", objCnt)
		for i := 0; i < objCnt; i++ {
			ifIdx := int(bulkInfo.LagList[i].LagIfIndex)
			ent := server.lagPropMap[ifIdx]
			ifIndexList := bulkInfo.LagList[i].IfIndexList

			for i := 0; i < len(ifIndexList); i++ {
				port := ifIndexList[i]
				portEnt := server.portPropMap[int(port)]
				portEnt.LagIfIdx = ifIdx
				server.portPropMap[int(port)] = portEnt
				ent.PortMap[int(port)] = true
			}
